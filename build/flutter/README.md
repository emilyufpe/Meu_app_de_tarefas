...
* `xquest` - project identifier (lowercase, no spaces, i.e. "snake_case") used for bundle IDs and package names.
* `` - project description.
* `xquest` - executable or bundle name on disk.
* `XQuest` - display name shown in window titles and about app dialogs.
* `com.emilylima` - org name in reverse domain name notation, e.g. `com.mycompany.myproject`.
